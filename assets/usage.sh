...
ep="/bin/bash -c $s"
yellow_i "--- $s\n"
i "... mysqldump to a local file or S3 location, optionally gzipped"
i "    If dumping to a local file, mount a host volume to access"
i "    the local file after the container exits."
cat << EOF

    # e.g. ... dump my_db with some mysqldump options, to host path /my/dir/dump.sql.gz
    # Here we mount /my/dir to /project in container.
    #
    export DEST_PATH=/project/dump.sql.gz # output path INSIDE container.
    export DB_HOST="localhost" DB_USER="bob" DB_PASS="\${MY_PASSWORD}"
    export GZIP=true
...
s=/dump_to_s3.sh
ep="/bin/bash -c $s"
yellow_i "--- [DEPRECATED] $s\n"
i "... mysqldump to an S3 location, optionally gzipped"
i "    The file can be in S3, or available to container e.g. via a volume or docker copy"
cat << EOF
